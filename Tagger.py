...
async def start(event):
  await event.reply("**👋 **Salam** \n\n**💬 Mən sizin qurupunuzda istifadəçiləri çağırmağınız üçün yaradılmış çox funksiyanal botam**\n\n**✅ Botun istifadə qaydasını öyrənmək üçün**\n\n/help əmrindən istifadə edin**",
            buttons=(
		      ...
		      [Button.url('➕ Məni Qurupa əlavə et ➕', 'http://t.me/Ballastaggerbot?startgroup=a')],
                      [Button.url('Support 🤝', 'https://t.me/Ballasiceat')],
                      [Button.url('Rəsmi Kanal 📢', 'https://t.me/Ballasicrat')],
...
async def help(event):
  ...
  await event.reply(helptext,
                    buttons=(
                      [Button.url('𝐁𝐀𝐋𝐋𝐀𝐒 | ༒︎𝐑𝐀𝐌𝐎༒︎ ', 'https://t.me/BOT_RAMO')],
                    ),
                    link_preview=False
                   )
...
async def mentionall(event):
  ...
  if mode == "text_on_reply":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      usrtxt += f"[{random.choice(sehidler)}](tg://user?id={usr.id}) "
      if event.chat_id not in anlik_calisan:
        await event.respond("Tag prosesi uğurla dayandırıldı ✅\n\n** [Button.url('Grupum😋💬', 'https://t.me/Ballasresmi']) Buda sizin reklamınız ola bilər @BOT_RAMO**✅")
        return
      if usrnum == 5:
...
print(">> Bot aktifdi bot hakda məlumatı @BOT_RAMO dan ala bilərsən Versiya 1.7.5<<")
